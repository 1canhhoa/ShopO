...
let initWebDeck =(app) => {
  const encodedToken = (payload) => {return jwt.sign({iss:"tohien",sub:payload,},process.env.JWT_ACCESSTK_SECRET,{expiresIn:"60m"})}
  router.post("/create-user" , upload.single("file") ,async (req, res, next) => {
    try { 
      const { username,password,email } = req.body;
      const checkUsername = await User.findOne({ username });
      const checkEmail = await User.findOne({ email });
      if(!username) {return next(new ErrorHandler("Please fill in your useranme", 400))}
      ...
      const accessToken = encodedToken({username,email,password,avatar})
  ...
  router.post("/activation",catchAsyncErrors(async(req, res, next) => {
      try {        
        ...
        sendToken(user,encodedToken(user),201,res)
      } catch (error) {
